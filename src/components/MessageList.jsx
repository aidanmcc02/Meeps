  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    const sameSender =
      (msg.senderId != null &&
        ...
        Number(msg.senderId) === Number(current.senderId)) ||
      (msg.sender === current.sender && current.sender != null);
    const lastInRun = current.messages[current.messages.length - 1];
    const lastTime = lastInRun?.createdAt
      ? new Date(lastInRun.createdAt).getTime()
      : 0;
...
function MentionSpan({
  ...
  return (
    <span
      ...
      onMouseEnter={(e) => {
        if (onMentionHover)
          onMentionHover(slug, e.currentTarget.getBoundingClientRect());
      }}
      onMouseLeave={() => {
...
function MessageLink({
  ...
  if (isMention) {
    const slug =
      href.replace(/^[^:]*mention:/i, "").trim() || href.slice(8).trim();
    return (
      <MentionSpan
        slug={slug}
...
function MessageList({
  ...
  const mentionHandlersRef = useRef({
    setHoveredMention: () => {},
    resolveMentionProfile: () => null,
  });
  ...
  useEffect(() => {
    ...
    const lastKey = lastMsg
      ? (lastMsg.id ?? `${lastMsg.sender}-${lastMsg.createdAt}`)
      : null;
    ...
    const onDocMouseOut = (e) => {
      ...
      const to =
        e.relatedTarget && typeof e.relatedTarget.closest === "function"
          ? e.relatedTarget.closest("[data-mention]")
          : null;
  ...
  const handleRootMouseOver = (e) => {
    ...
    const slug = mentionEl
      ...
      : (() => {
          const h = (mentionLink.getAttribute("href") || "").replace(
            /^mention:/i,
            "",
          );
          return h;
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 dark:text-gray-500">
            ...
            <p className="max-w-md text-xs">
              Start the conversation by sending a message. Markdown is
              supported.
            </p>
          </div>
        )}
          ...
          ({ dateKey, messages: dayMessages }) => (
            <div key={dateKey} className="space-y-3">
              ...
              {groupConsecutiveBySender(dayMessages).map((run) => {
                ...
                const profile =
                  first.senderId != null ? profiles[first.senderId] : null;
                const avatarUrl =
                  profile?.avatarUrl ??
                  senderNameToAvatar[first.sender] ??
                  null;
                const initials =
                  (first.sender || "?")
                    .split(" ")
                ...
                return (
                  <div key={runKey} className="space-y-0">
                    {run.messages.map((msg, runIndex) => {
                      ...
                      const isMentioned = messageMentionsCurrentUser(
                        msg.content,
                        currentUserName,
                        mentionSlugToName,
                      );
                      ...
                      return (
                        <div
                          key={
                            msg.id ??
                            `${msg.sender}-${msg.createdAt}-${runIndex}-${msg.content}`
                          }
                        ...
                        >
                          {showHeader ? (
                            onSenderClick &&
                            (first.senderId != null || first.sender) ? (
                              <button
                                type="button"
                                onClick={() =>
                                  onSenderClick({
                                    ...
                                    displayName:
                                      first.sender || profile?.displayName,
                                    name: first.sender,
                                  })
                                }
                            ...
                            <div
                              className="w-8 flex-shrink-0 self-stretch min-h-[2px]"
                              aria-hidden="true"
                            />
                          )}
                          <div className="flex-1 min-w-0 relative">
                            {showHeader ? (
                              <div className="flex items-baseline gap-2 flex-1 min-w-0">
                                ...
                                {msg.createdAt && (
                                  <span
                                    ...
                                    title={formatMessageTimeTooltip(
                                      msg.createdAt,
                                    )}
                                  >
                              ...
                              <div className="relative h-0 overflow-visible">
                                <div
                                  className="absolute right-0 top-0 opacity-0 group-hover:opacity-100 transition-opacity z-10"
                                  ref={openMenuId === msg.id ? menuRef : null}
                                >
                                  <button
                                    type="button"
                                  ...
                                  (() => {
                                    ...
                                    return legacyEmbed ? (
                                      ...
                                      <div className="prose prose-sm dark:prose-invert max-w-none break-words [&_img]:my-1 [&_img]:max-h-48 [&_img]:rounded-lg [&_.mention]:font-medium [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-0.5 [&_strong]:font-bold [&_strong]:text-inherit [&_em]:italic [&_del]:line-through [&_a]:text-indigo-600 [&_a]:dark:text-indigo-400 [&_a]:hover:underline">
                                        {splitContentSegments(msg.content).map(
                                          (seg, i) =>
                                            seg.type === "mention" ? (
                                              <MentionSpan
                                                key={i}
                                  ...
                                  ((msg.reactions &&
                                    ...
                                    >
                                      {Object.entries(msg.reactions || {}).map(
                                        ([emoji, userIds]) => {
                                          const count = Array.isArray(userIds)
                                            ? userIds.length
                                            : 0;
                                          const hasMe =
                                            currentUserId != null &&
