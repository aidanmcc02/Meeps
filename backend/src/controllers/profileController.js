...
function mapUserRowToProfile(row) {
  ...
  const activityDetailLevel =
    detailLevel === "just_application" || detailLevel === "none"
      ? detailLevel
      : "in_depth";
  ...
  try {
    ...
    const result = await db.query(
      ...
      [
        ...
        newLoseGifUrl,
        newTheme,
        newActivityLoggingEnabled,
        newActivityDetailLevel,
        newDoNotDisturb,
      ],
    );
    ...
    try {
      if (
        ...
        previousAvatarPublicId !== nextAvatarPublicId
      ) {
        await db.query(
          "UPDATE uploads SET is_pinned = FALSE WHERE public_id = $1",
          [previousAvatarPublicId],
        );
      }
      if (nextAvatarPublicId) {
        await db.query(
          "UPDATE uploads SET is_pinned = TRUE WHERE public_id = $1",
          [nextAvatarPublicId],
        );
      }
    } catch (pinErr) {
