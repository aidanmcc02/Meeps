...
async function linkAccount(req, res) {
  try {
    ...
    if (err.response?.status === 403) {
      return res
        .status(502)
        .json({ message: "Riot API key invalid or rate limited" });
    }
...
async function getPlayerStats(req, res) {
  try {
    ...
    if (linkResult.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Player has not linked Valorant" });
    }
        ...
        if (summary) {
          matches.push({
            ...
            startTime: summary.startTime
              ? new Date(parseInt(summary.startTime, 10)).toISOString()
              : null,
          });
        }
      } catch (e) {
    ...
    const riotMsg =
      err.response?.data?.message || err.response?.data?.status?.message;
    let message = "Failed to load stats";
    if (err.message === "RIOT_API_KEY is not set") {
      message = "Server: Riot API key not configured.";
    } else if (status === 403) {
      message =
        "Riot API returned 403 Forbidden. Use a Production API key (dev keys like RGAPI-... expire in 24h and often cannot access Valorant match data). Enable Valorant for your app at developer.riotgames.com.";
    } else if (status === 429) {
  ...
  try {
    ...
    if (!actId) {
      ...
      if (!actId) {
        ...
        console.log(
          "[Valorant leaderboard] No act ID from content. Keys:",
          keys.join(", "),
        );
        return res.json({
          actId: null,
          ...
          actName: null,
          error:
            "No current act in content. Try ?actId=<act-uuid> if you have one.",
        });
      }
    }
    ...
    if (content) {
      const acts =
        content.acts ||
        content.Acts ||
        content.Seasons?.filter((s) => (s.Type || s.type) === "act") ||
        [];
