...
exports.notify = async (req, res, next) => {
  ...
  console.log(
    "[build-notify] Request received. BUILD_WEBHOOK_SECRET configured:",
    !!secret,
    "| Received secret present:",
    !!receivedSecret,
    "| Match:",
    secret ? receivedSecret === secret : "n/a",
  );
  if (!secret) {
    console.log(
      "[build-notify] Not running: BUILD_WEBHOOK_SECRET is not set on the server (add it in Railway/backend env vars).",
    );
    return res
      .status(503)
      .json({ error: "Build notifications not configured" });
  }
  if (receivedSecret !== secret) {
    console.log(
      "[build-notify] Not running: secret mismatch. Ensure BUILD_WEBHOOK_SECRET in GitHub Secrets matches the value set on the backend.",
    );
    return res.status(401).json({ error: "Invalid build secret" });
  }
  console.log(
    "[build-notify] Checks passed. Body:",
